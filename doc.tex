...
The value of NHST is that asking whether we can reject the null hypothesis is a proxy for asking whether we clearly \emph{how} our data differs from it. For example, in a t-test, we are nominally asking whether we can see a \emph{difference} between two means, but the scientific question is whether we a confident which of the two means is \emph{larger}; similarly, tests for whether two values are correlated are a proxy for whether we can see the sign of the correlation coefficient \citep{robinson2001past}. In other cases (e.g., a one-way ANOVA), it may not be simple to describe the difference we see, but NHST is still a reasonable, accepted way to evaluate whether an effect has been seen clearly.
...
Descriptions of statistical results using the language of clarity should begin with reference to the effect. For example, ``The difference between the control and treatment group was not statistically clear.'' Table~\ref{quotetab} shows published examples of statements that misinterpret \pvals in three different ways and demonstrates how to rephrase them in the language of clarity. We have attempted to do this thoughtfully, and therefore the language on the right differs from the language on the left in many ways. The point is not that changing ``significance'' to ``clarity'' is a substantial improvement by itself, but that it can prompt rethinking and reinterpretation.

\subsection*{Caveats}

The suggested change in language should help focus attention on improved statistical practice, but does not guarantee it. In particular, there is a danger that ``statistical clarity'' will eventually come to be seen as end to itself, the way that ``significance'' is now. The idea of ``statistical clarity'' will work best if it remains linked to the principles of attaching clarity to the study, not the system, and of focusing on effects.

The use of statistical clarity could be confounded with other meanings of the word clarity. We have found that scientists' understand of our proposed use of statistical clarity is enhanced by connecting clarity statments to statements about P values. 

ADD a bit about winners' curses, plausibility and the like here (and take some of it out of the Conclusions.
