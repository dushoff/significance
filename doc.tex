...
\textbf{Abstract}: Null hypothesis significance testing (NHST) remains popular despite decades of concern about misuse and misinterpretation.
Here we argue that a significant part of the problem is due simply to language: significance testing has little to do with other meanings of the word commonly uis not about  whatever the use of have led to debates over whether to abandon its use. 
What to say about when and how they are useful? 
Conflation?
Get to the point!
Part of the problem here is the use of the word ``significance''. 
While hypothesis testing may be Here, we argue that statistical hypothesis tests are useful, but are not, and should not
be referred to, as tests of significance. We propose a simple method to reduce misconceptions about NHST and p~values: 
refer to the results of NHST (e.g. $p <$ or $> 0.05$) using ``statistical clarity'' rather than ``statistical significance''. 
A test of the null hypothesis tells 
