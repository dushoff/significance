...
We believe that a significant \mk{change word?} part of the problem is due simply to language: significance testing has little to do with other meanings of the word ``significance''. \mk{Broken sentence:} commonly uis not about  whatever the use of have led to debates over whether to abandon its use. 
...
\textbf{Signs}. Who has said before that we test zero \emph{because} we want to know if we can see a sign clearly? If nobody, how much time do we want to spend on that point?
...
As an example, consider \jd{Should I write a paragraph about signs and clarity?}
